...
def slow(
    ...
    task = JumpPlanarWalker3(x_vel_limit=0.5, random=random)
...
def vel_1(
    ...
    """Returns the Stand task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = JumpPlanarWalker3(x_vel_limit=1, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
def vel_2(
    ...
    """Returns the Stand task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = JumpPlanarWalker3(x_vel_limit=2, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
def vel_3(
    ...
    """Returns the Stand task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = JumpPlanarWalker3(x_vel_limit=3, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
class JumpPlanarWalker3(base.Task):
    ...
    def __init__(self, x_vel_limit, random=None):
        ...
        super(JumpPlanarWalker3, self).__init__(random=random)
    ...
    def initialize_episode(self, physics):
        ...
        super(JumpPlanarWalker3, self).initialize_episode(physics)
    ...
    def get_reward(self, physics):
        ...
        reward = x_vel_reward + angle_reward + delta_h_penalty + ctrl_penalty + alive_reward + foot_penalty + foot_diff_penalty + thigh_diff_penalty + \
                leg_diff_penalty + not_jump_penalty + torso_height_reward + torso_z_speed_reward + right_position_penalty + torso_position_penalty
